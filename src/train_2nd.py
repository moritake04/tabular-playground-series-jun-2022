...
def main():
    ...
    added_data = pd.read_csv(cfg["general"]["added_data_name"])
    ...
    for feature in tqdm(f4_features):
        ...
        added_data_f4 = added_data.loc[:, ["row_id"] + f4_features]
        test = added_data_f4.loc[data_f4[feature].isnull()]
        if len(test) == 0:
            continue
        train = added_data_f4.loc[(data_f4[feature].notnull())]
